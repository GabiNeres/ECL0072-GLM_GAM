...
Antes de efetuar a análise exploratória, é importante que cada variável do banco de dados seja caracterizada de acordo com sua natureza (*character*, *factor*, *numeric*, *integer*, etc.).

Olhando a estrutura dos dados, percebemos que precisamos converter as variáveis *character* para *factor*
...
- $H_1:$ A probabilidade de sobrepeso difere entre as difrerentes espécies, com a espécie Gentoo apresentando as maiores probabilidades
...
aic %>% 
  arrange(AIC) %>%
  kable(format = "html", caption = "Comparação de modelos via AIC") %>%
...
res$coefficients %>%
  round(.,2) %>%
  as.data.frame() %>% 
  kable(format = "html", caption = "Sumário estatístico do modelo escolhido") %>%
  kable_paper(bootstrap_options = c("striped","condensed"), full_width = F) %>%
  kable_classic()
