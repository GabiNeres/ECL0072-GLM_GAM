...
Versão reparametrizada $(\mu, \tau)$:

$$\large \mu = \frac{\alpha}{\alpha+\beta}, \quad \tau = \alpha + \beta$$
$$\large \alpha = \mu \tau, \quad \beta = \tau(1-\mu)$$
...
$$\large Y \sim Beta(\mu\tau, ~\tau(1-\mu))$$

$$\large E(Y) = \mu, \quad Var(Y) = \frac{\mu(1-\mu)}{1+\tau}$$
...
$\large logit(\mu) = log(\frac{\mu}{1-\mu}); \quad \mu = \frac{e^\eta}{1 + e^\eta}$

---
## <span style="color: #ee6c4d;">Distribuição Beta </span>

A distribuição Beta também pode ser usada para variáveis que estão limitadas em outro intervalo de valores

- Redimensionar os valores para o intervalo (0,1) conforme:

$$\large z_i = \frac{y_i-a}{b-a}$$
...
Dados de contagens muitas vezes apresentam um excesso de zeros
...
$$\large f(y_i|\theta) = \frac{\lambda^{y_i}e^{-\lambda_i}}{y_i!}$$
--
...
$$P(Y_i = 0) = \pi_i + (1-\pi_i) \times \frac{\lambda^{0} e^{-\lambda_i}}{0!}$$
]
...
$$P(Y_i = 0) = \pi_i + (1-\pi_i) \times \frac{\lambda^{0} e^{-\lambda_i}}{0!}$$
Portanto:
$$P(Y_i = 0) = \pi_i + (1-\pi_i) \times  e^{-\lambda_i}$$
...
$$P(Y_i = y_i) = (1-\pi_i) \times  \frac{\lambda^{y_i} e^{-\lambda_i}}{y_i!}$$
...
$E(Y_i) = \lambda_i (1-\pi_i)$

$Var(Y_i) = (1-\pi_i)(\lambda_i + \pi_i\lambda^{2}_i)$
...
$\large logit(\pi_i) = \eta_i= \beta_0 + \sum_{j=1}^{p} \beta_j x_{ij}$

$\large log(\lambda_i) = \eta_i = \gamma_0 + \sum_{j=1}^{p} \gamma_j z_{ij}$
]
...
$$\large P(Y_i = y_i) =  \left\{ \begin{array}{lr} \pi_i + (1-\pi_i) \times f_{NB}(y_i=0|\theta) & y_i = 0 \\ (1-\pi_i) \times f_{NB}(y_i|\theta) & y_i > 0 \end{array}\right.$$
...
$Var(Y_i) = (1-\pi_i)(\mu_i + \frac{\mu^2_i}{k})~ +~ \mu^2_i \times (\pi^2_i + \pi_i)$
...
onde:
- $p_i:$ probabilidade da ausência da espécie
...
$$\large f(y_i; \lambda_i) = \frac{\lambda^{y_i}  e^{-\lambda_i}}{y!}$$
--
...
$$\large f(y_i > 0; \lambda_i) = \frac{\lambda^{y_i} e^{-\lambda_i}}{(1-e^{-\lambda_i}) y!}$$
...
$$\large f(y_i > 0; \mu_i, k) = \frac{\frac{\Gamma(y_i+k)}{\Gamma(k)\Gamma(y_i+1)}  \left (\frac{k}{\mu_i + k}\right)^k \left(1 - \frac{k}{\mu_i + k}\right)^{y_i}} {1 - \left (\frac{k}{\mu_i + k}\right)^k}$$
...
Escolher uma das três distribuições apresentadas:


**salamanders.rds**
- Abundância (contagem) de diversas espécies de salamandras
  - Associar a fatores de mineração


**fisheries.rds**
- Razão de captura incidental (bycatch / captura espécies-alvos)
  - Associar a razão de bycatch com características oceanográficas, espaço-temporais e/ou fatores operacionais da pesca

- Abundância de espécies não-alvo (e.g., *Isurus_oxyrinchus*, *Ruvettus_pretiosus*, *Makaira_nigricans*)
