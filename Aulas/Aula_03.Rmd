...
**Livros**
- **Caps. 8 & 10:** Zuur et al. (2009)
- **Cap. 4 (seção 4.5):** Bolker (2008)
- **Caps. 2 & 3:** Faraway (2016)
- **Cap. 13:** Gelman et al. (2020)
- **Caps. 1 & 5:** Agresti (2015)*
...
$$\color{Tomato}{g(\mu_i)} = \color{Tomato}{\eta_i} = \beta_0 + \sum_{j=1}^{p} \beta_j x_{ij}$$
...
- <span style="color: #ee6c4d;"><b>`1º componente:`</b></span> Distribuição de probabilidade para variável resposta
  ...
  - **A família exponencial de distribuições** 
     - Cobre uma diversidade de distribuições de probabilidade
          - Específic1 à natureza da variável $y_i$
...
Assim como nos modelos lineares, os GLMs devem atender aos pressupostos de:
...
- a normalidade residual só se aplica para o caso em que a variável resposta ($y$) segue uma distribuição Normal de probabilidade
  - os resíduos podem ser avaliados sob o prisma do modelo de probabilidade escolhido para a variável $y$ 
...
Os dados de pinguins de Palmer contêm a informação sobre o estado de saúde dos indivíduos amostrados:
...
A Bernoulli é um caso especial da Binomial quando $n = 1$
</p>
...
Em modelos com distribuição binomial, a função de ligação logit é a mais amplamente utilizada
  - Há outras funções de ligação que podem ser testadas:
    - probit
    - cloglog
    - cauchit
...
--
  ...
  - Pode-se testar o mesmo modelo usando funções de ligação diferentes
...
Dados que podem ser utilizados para elaboração do 1º capítulo do relatório:


**koalas.rds**

- Presença/ausência de coalas na Austrália
  - Associar a diferentes fatores de urbanização e qualidade de habitat

----

**fisheries.rds**

- Presença/ausência de diferentes espécies
  - Associar a presença de uma espécie à características oceanográficas
      - Transformar as abundâncias em variável binária (se y> 0, atribui-se 1; caso contrário, 0 (y=0))